...
3. Keep OpenClaw bound to loopback (`gateway.bind=loopback`) — not `0.0.0.0` / `lan`. Cursor should use an SSH tunnel to `127.0.0.1:18789`.
...
| `OPENCLAW_GATEWAY_TOKEN` | yes | Gateway bearer token (**stan** config only) |
| `XAI_API_KEY` | yes for Grok | xAI Console API key (`https://console.x.ai`) |
...
## Day-2 VPS maintenance
See [`deploy/openclaw-vps-ops.md`](./openclaw-vps-ops.md) for the safe OpenClaw update path, Grok/xAI wiring, and the average security baseline (UFW, SSH, Telegram pairing).
