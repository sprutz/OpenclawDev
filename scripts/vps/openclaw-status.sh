...
timeout 40 sudo -u stan -H bash -lc "export XDG_RUNTIME_DIR=/run/user/1001; openclaw gateway status" || true
echo
echo "--- stan channels (probe, 45s cap) ---"
timeout 45 sudo -u stan -H bash -lc "export XDG_RUNTIME_DIR=/run/user/1001; openclaw channels status --probe" 2>/dev/null || echo "(channels probe timed out or unavailable)"
echo
echo "--- token fingerprint (sha12 only) ---"
python3 - <<'"'"'PY'"'"'
...
root_path = "/root/.openclaw/openclaw.json"
if os.path.exists(root_path):
    root = json.load(open(root_path))["gateway"]["auth"]["token"]
    print("root_sha12", hashlib.sha256(root.encode()).hexdigest()[:12], "(stale conflict — should be removed)")
else:
    print("root_config=absent")
PY
'
